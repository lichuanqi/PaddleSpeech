...
class U2Infer():
    ...
    def run(self, audio_path):
        ...
        check(self.audio_path)

        with paddle.no_grad():
            # read
            audio, sample_rate = soundfile.read(
                self.audio_path, dtype="int16", always_2d=True)
...
if __name__ == "__main__":
    ...
    if 'deepspeech2' in args.config:
    ...
    elif 'conformer' in args.config:
        logger.info('开始预测，使用 conformer 模型')
        t_init_s = time.time()
        conformer = U2Infer(config, args.checkpoint_path, args.ngpu)
        t_init_e = time.time()
        logger.info('初始化完成，用时:{}'.format(t_init_e-t_init_s))
        ...
        for i in range(num):
            ...
            #     audio_data = base64_bytes.decode('utf-8')
            # encode_te = time.time()
            # logger.info("b64encode time %f s." % (encode_te - encode_ts))
            # u2_predicit.run_wav_data(audio_data)
