...
class Recorder:
    ...
    def stop_save(self):
        ...
        """
        # 保存路径及名称
        savename = os.path.join(self.path_wavs, f'{self.post_id}.wav')
        ...
        if self._running:
            ...
            msg = get_time() + f" 录音结束,保存至 {savename}\n"
            self.result_text.insert('end', msg)
    ...
    def http_predict(self):
        ...
        msg = get_time() + f" 调用接口: {server_url}\n"
        self.result_text.insert('end', msg)
        ...
        try:
            ...
            re_tn = datetime.now()
            msg = get_time() + " 接口响应时间: %.2f s.\n" % (re_tn - re_ts)
            self.result_text.insert('end', msg)

            res = data['result']['transcription']
            msg = get_time() +  f" 语音识别结果: {res}\n"
            self.result_text.insert('end', msg)

            # 结果保存至txt
            
            result_to_txt(self.path_results, self.post_id, res)
            msg = get_time() + f'结果已保存至: {self.path_results}/{self.post_id}.txt'
            self.result_text.insert('end', msg)

        # 无法连接接口
        except requests.exceptions.ConnectionError as err:
            msg = get_time()  + str(err) + '\n'
            self.result_text.insert('end', msg)


def result_to_txt(self, savapath, post_id, result):
    """把语音识别结果保存为txt文件
    ...
    Params
        ...
        result
    Output
        savapath/post_id.txt
    """
    savename = os.path.join(savapath, f'{post_id}.txt')
    with open(savename, 'w', encoding='utf-8') as f:
        f.write(post_id + ',' + result)
