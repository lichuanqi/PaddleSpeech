...
if __name__=='__main__':
    ...
    # text='今天的天气不错啊'
    text='demos/text_to_speech/address_10_text.txt'
    output='demos/text_to_speech/address_10/'
    ref_savepath='demos/text_to_speech/address_10_ref.txt'
    am='fastspeech2_csmsc'
    am_config='configs/fastspeech2_csmsc-zh/1.0/fastspeech2_nosil_baker_ckpt_0.4/default.yaml'
    am_ckpt='configs/fastspeech2_csmsc-zh/1.0/fastspeech2_nosil_baker_ckpt_0.4/snapshot_iter_76000.pdz'
    am_stat='configs/fastspeech2_csmsc-zh/1.0/fastspeech2_nosil_baker_ckpt_0.4/speech_stats.npy'
    spk_id=0
    phones_dict='configs/fastspeech2_csmsc-zh/1.0/fastspeech2_nosil_baker_ckpt_0.4/phone_id_map.txt'
    tones_dict=None
    speaker_dict=None
    voc='pwgan_csmsc'
    voc_config='configs/pwgan_csmsc-zh/1.0/pwg_baker_ckpt_0.4/pwg_default.yaml'
    voc_ckpt='configs/pwgan_csmsc-zh/1.0/pwg_baker_ckpt_0.4/pwg_snapshot_iter_400000.pdz'
    voc_stat='configs/pwgan_csmsc-zh/1.0/pwg_baker_ckpt_0.4/pwg_stats.npy'
    lang='zh'
    ...
    for i in range(len(data)):
        logger.info('开始处理: {}'.format(data[i]))
        save_path = output + 'tts_' + str(i) + '.wav'
        tts_executor.infer(text=data[i], lang=lang, am=am, spk_id=spk_id)
        res = tts_executor.postprocess(output=save_path)
        logger.info('音频文件保存至: {}'.format(res))

        tn = TN.normalize(data[i])[0]
        ref = '{}\t{}'.format(res, tn)
        refs.append(ref)

    # 将文件路径和文字结果保存至列表
    with open(ref_savepath, 'w') as f:
        for ref in refs:
            f.write(ref+'\n')
    logger.info('音频文件和文字对应结果保存至: {}'.format(ref_savepath))